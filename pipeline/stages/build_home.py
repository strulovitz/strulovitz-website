...
WHAT IT WRITES
It replaces everything between these two lines in site/index.html:
    <!-- RESULTS BEGIN -->
    <!-- RESULTS END -->
...
HOME_PAGE = REPO_ROOT / "site" / "index.html"
...
def build_html(rows: list[dict]) -> str:
    ...
    out.append(
        '  <p class="note">Clicking a node inside a world does not open an article yet. '
        'The reading pages and the illustrations are the next thing being built.</p>'
    )
...
def main() -> int:
    ...
    if BEGIN not in page or END not in page:
        print(f"ERROR: {HOME_PAGE} has lost its {BEGIN} / {END} markers.")
        return 1

    before = page[:page.index(BEGIN)]
    after = page[page.index(END) + len(END):]
    HOME_PAGE.write_text(before + build_html(rows) + after, encoding="utf-8")

    print(f"Wrote the first-results section for {len(rows)} editions into {HOME_PAGE.name}.")
    for row in rows:
        ...
        try:
            with connect() as db:
                log_job(db, action_type="export", actor="claude-opus-5",
                        plain_words=(
                            f"Wrote the first-results section of the home page from the "
                            f"{len(rows)} editions on disk: a button for each edition a "
                            f"visitor can fly through, and a table of what each model made "
                            f"of the same stories, with every number measured rather than "
                            f"estimated. It is generated rather than typed, so adding a "
                            f"model or a story updates it by itself."),
                        outputs=[str(HOME_PAGE.relative_to(REPO_ROOT))])
