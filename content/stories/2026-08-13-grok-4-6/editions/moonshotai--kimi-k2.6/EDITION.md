# xAI's Grok 4.6 catches up to OpenAI and Anthropic on coding benchmarks

**Kimi K2.6** (Moonshot) — its own edition of *2026-08-13-grok-4-6*
...
- Grok 4.6 is priced at $2 per million input tokens and $6 per million output tokens, undercutting rivals GPT 5.6 Soul and Fable 5 on output pricing.
  — https://www.youtube.com/watch?v=rdYBjpylJUQ
- On GPTEval, Grok 4.6 High scored first, ahead of GPT 5.6 Soul and Fable 5 Max.
  — https://www.youtube.com/watch?v=rdYBjpylJUQ
- On DeepSWE, Grok 4.6 placed third behind GPT 5.6 Soul Max and Fable 5.
  — https://www.youtube.com/watch?v=rdYBjpylJUQ
- The model is an iterative improvement on Grok 4.5, built on a 1.5 trillion parameter base with a longer supplemental training run that included Cursor-derived data.
  — https://www.youtube.com/watch?v=VB39bHByHuU
- Grok Bot runs agents on cloud virtual machines that remain active 24/7, with a 'teach a task' feature for recording workflows.
  — https://www.youtube.com/watch?v=VB39bHByHuU
- xAI acquired Cursor in April 2026, gaining access to coding data that sources describe as central to Grok's recent improvements.
  — https://www.youtube.com/watch?v=rdYBjpylJUQ
- Elon Musk stated Grok 4.7 will be ready in 3 to 4 weeks with supplemental SpaceX data training, and Grok 5 is targeted before year-end.
  — https://www.youtube.com/watch?v=VB39bHByHuU
- During launch week, usage in Cursor and Grok Build is doubled.
  — https://www.youtube.com/watch?v=VB39bHByHuU
...
`supplemental-training-run` — 126 words

A supplemental training run is additional training applied to a base model after its initial creation. In machine learning, models are typically trained in stages: first, a large base model is trained on broad data, then smaller, targeted training runs refine it for specific tasks or improve particular capabilities. The base model's parameters—its 'knowledge'—remain largely intact, but the supplemental run adjusts how it responds, reasons, or behaves in specific domains. In the case of Grok 4.6, xAI took the same 1.5 trillion-parameter base used for 4.5 and ran a longer supplemental phase that included curated engineering data, model-generated reasoning traces, and an improved optimizer. This approach is faster and less expensive than training a new base model from scratch, and it allows companies to iterate quickly.

### Agent (in AI systems)
`agent-in-ai-systems` — 131 words
...
`xai`  `grok`  `frontier models`  `coding`  `ai agents`
